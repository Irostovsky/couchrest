...
module RestClient 
  ...
  def self.post(uri, payload, headers=nil)
    start_query = Time.now
    log = {:method => :post, :uri => uri, :payload =>  (payload ? (MultiJson.load(payload) rescue 'parsing error') : nil), :headers => headers}
  ...
  def self.put(uri, payload, headers=nil)
    start_query = Time.now
    log = {:method => :put, :uri => uri, :payload => (payload ? (MultiJson.load(payload) rescue 'parsing error') : nil), :headers => headers}
    response = super(uri, payload, headers=nil)
    end_query = Time.now
    log[:duration] = (end_query - start_query)
    CouchRest::Logger.record(log)
